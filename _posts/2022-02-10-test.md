...
* R : reward function $ R_{s} = \mathrm{E}[r_{t+1},s_t=s] $
...
$$ v(s) = \mathrm{E}[R_t|s_t=s] \newline= \mathrm{E}[r_{t+1} + \gamma r_{t+2} + \gamma^{2}r_{t+3} + ... |s_t=s] \newline= \mathrm{E}[r_{t+1} + \gamma R_{t+1}|s_t=s] $$
...
$$ v = \mathrm{R} + \gamma \mathrm{P} \newline v = (1-\gamma \mathrm{P})^{-1}\mathrm{R} $$
